<template>
  <q-page class="q-pa-md">
    <div class="row q-mb-md">
      <div class="col">
        <div class="text-h5">Gestion des Remises Timbres</div>
        <div class="text-caption text-grey-7 q-mt-xs">
          <q-icon name="info" size="16px" color="grey-7" />
          Le trésor remet les timbres à la mairie
...
import { db, type TimbresRemise, type TimbresValeurs, type TimbresQuotite } from 'src/database/db';
...
const form = ref<Partial<TimbresRemise>>({
  date: new Date(),
...
function exportCsv() {
  exportToCsv(filteredRemises.value as Record<string, unknown>[], columns, 'timbres-remises');
}
...
async function loadData() {
  loading.value = true;
  try {
    remises.value = await db.timbresRemises.toArray();
  } catch (error) {
...
const openDialog = (remise?: TimbresRemise) => {
  ...
  if (remise && remise.id) {
    ...
    form.value = {
      date: new Date(),
      exercice: new Date().getFullYear(),
      numeroRemise: `REM-T-${new Date().getFullYear()}-${nextNum.toString().padStart(4, '0')}`,
      type: 'Remise',
      timbres: empty,
      total: 0,
      observations: '',
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (isEditing.value && editingId.value) {
      await db.timbresRemises.update(editingId.value, {
        ...data,
        updatedAt: now,
      });
      $q.notify({
      ...
      await db.timbresRemises.add(data);
      $q.notify({
        type: 'positive',
...
const confirmDelete = (remise: TimbresRemise) => {
  $q.dialog({
    title: 'Confirmation',
    ...
    void (async () => {
      try {
        if (remise.id) {
          await db.timbresRemises.delete(remise.id);
          $q.notify({
            type: 'positive',
...
const viewDetails = (remise: TimbresRemise) => {
  openDialog(remise);
};
