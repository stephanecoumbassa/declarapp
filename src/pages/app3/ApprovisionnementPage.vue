<template>
  <q-page class="q-pa-md">
    <div class="row q-mb-md">
      <div class="col">
        <div class="text-h5">Approvisionnement Timbres</div>
        <div class="text-caption text-grey-7 q-mt-xs">
          <q-icon name="info" size="16px" color="grey-7" />
          L'imprimerie envoie des timbres au trésor
...
const form = ref<Partial<TimbresApprovisionnement>>({
...
async function loadData() {
  loading.value = true;
  try {
    approvisionnements.value = await db.timbresApprovisionnements.toArray();
  } catch (error) {
...
function exportCsv() {
  exportToCsv(
    filteredAppros.value as Record<string, unknown>[],
    columns,
    'timbres-approvisionnements',
  );
}
...
const openDialog = (appro?: TimbresApprovisionnement) => {
  ...
  if (appro && appro.id) {
    ...
    if (appro.detailsQuotites) {
      ...
      for (const [prixStr, val] of Object.entries(appro.timbres)) {
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (isEditing.value && editingId.value) {
      await db.timbresApprovisionnements.update(editingId.value, {
        ...data,
        updatedAt: now,
      });
      $q.notify({
      ...
      await db.timbresApprovisionnements.add(data);
      $q.notify({
        type: 'positive',
...
const confirmDelete = (appro: TimbresApprovisionnement) => {
  $q.dialog({
    title: 'Confirmation',
    ...
    void (async () => {
      try {
        if (appro.id) {
          await db.timbresApprovisionnements.delete(appro.id);
          $q.notify({
            type: 'positive',
...
const viewDetails = (appro: TimbresApprovisionnement) => {
  openDialog(appro);
};
